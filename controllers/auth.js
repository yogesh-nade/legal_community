...
const { SECRET } = require('../utils/config'); // JWT secret from environment
...
const loginUser = async (req, res) => {
  ...
  res.status(200).json({
    ...
    id: user._id,
    avatar: user.avatar,
    karma: user.karmaPoints.postKarma + user.karmaPoints.commentKarma, // Combined karma
  });
};
...
const signupUser = async (req, res) => {
  ...
  res.status(200).json({
    ...
    karma: 0, // New user starts with 0 karma
  });
};
